---
read_when: "Synchronize animation markers with gameplay, effects, audio, or UI"
last_reviewed: 2026-06-17
---

# Integration, events, priorities, and cross-system patterns

## Marker-driven events

Use `GetMarkerReachedSignal("MarkerName")` to trigger events at authored times. Prefer it over polling `track.TimePosition` or using the older `KeyframeReached` event.
...
For predictive playback, the client starts the animation immediately. The server runs the authoritative version, and the client corrects or blends if the server disagrees.
