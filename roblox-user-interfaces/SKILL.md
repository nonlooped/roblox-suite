...
description: Exhaustive, modern guide to every Roblox GUI container type (ScreenGui with ScreenInsets/DisplayOrder/ResetOnSpawn, SurfaceGui with Adornee/Face/AlwaysOnTop/Brightness/LightInfluence/ZOffset/MaxDistance, BillboardGui with the same plus StudsOffset) and the complete set of building blocks (Frame, CanvasGroup, TextLabel, ImageLabel, TextButton, ImageButton, TextBox, ScrollingFrame, ViewportFrame, plus all layout and modifier objects: UIListLayout, UIGridLayout, UIPageLayout, UIStroke, UIGradient, UICorner, UIPadding, UIAspectRatioConstraint, UISizeConstraint, UIScale, etc.). Deep coverage of positioning/sizing/ZIndex/AnchorPoint/scale best practices, clipping, AutomaticSize, responsive design, interaction (Activated vs Mouse events, drag detectors), CoreGui control, universal styling, and especially "Particles in the UI" techniques (custom ImageLabel particle systems + TweenService, CanvasGroup effects, UIStroke shimmer, etc.). Note that real 3D ParticleEmitters, Beams, and Trails do not render inside ViewportFrame. Use for any HUD, menu, inventory, shop, nameplate, in-world terminal, floating marker, or 2D/hybrid interface work.
---
