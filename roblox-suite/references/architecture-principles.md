...
- Anything that reads or writes persistent state across sessions → data-persistence skill (and its references/).
- Motion, character actions, or UI transitions → animation-and-tweening (and its references/).
- Any 2D interface, HUD, menu, or "particle" effect inside UI → user-interfaces (and its references/, especially particles-in-ui.md).
- Robux purchases, passes, or shops → monetization-gamepasses.
- Fire, smoke, sparks, trails, or other 3D world VFX → visual-effects-and-particles.
- "How do I even structure this?" or "which service?" or "what can I actually store?" → fundamentals-and-services.
- "Is this safe on the client?" or "how do I securely call from client to server?" → client-server-networking-and-security.
